-- ========================================================
-- PROJECT: OPENxAll (V7 - Key Verification & Theme Restored)
...
ToggleBtn.Visible = false -- ซ่อนไว้รอผ่านคีย์
...
MainFrame.Visible = false -- ซ่อนไว้รอผ่านคีย์
...
-- ระบบคีย์ลัดปุ่ม Q : กดแล้วหายไปหมดทั้งยิ้มและปุ่ม O
...
local function createTab(name, id, pos)
    ...
    page.Visible = false
    page.CanvasSize = UDim2.new(0, 0, 4.2, 0) -- เพิ่มพื้นที่รองรับฟังก์ชั่นสีที่ดึงกลับมา
...
-- ================= [ หน้าที่ 2: Aimbot & Advanced 2D ESP + ดึงสี ESP กลับมา ] =================
local hitboxSize = 2
...
-- [ดึงกลับมา] หมวดตั้งค่าปรับสี ESP แบบกำหนดเอง
...
-- ================= [ หน้าที่ 3: เมนูช่วยเหลือ + ดึงปุ่มเปลี่ยนสี UI กลับมาครบเซ็ต ] =================
...
-- [ดึงกลับมา] หมวดปุ่มเปลี่ยนสี UI หน้าจอหลักอย่างรวดเร็ว
...
-- ================= [ หน้าที่ 4: เคลื่อนไหว/แมพ + ปืนก๊อปปี้อัจฉริยะ 2 จังหวะ ] =================
...
-- ================= [ 🔑 ระบบคีย์รักษาความปลอดภัยระดับสุดยอด (Key System Window) ] =================
...
KeyTitle.Text = "🔑 OPENxAll v7 -ระบบตรวจสอบคีย์"
...
KeySubmitBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == "OPEN" then
        -- ผ่านประตู! เปิดระบบทั้งหมด
        keyVerified = true
        KeyFrame:Destroy() -- ทำลายหน้าต่างคีย์ทิ้ง
        
        ToggleBtn.Visible = true -- เปิดปุ่มวงกลม O
        MainFrame.Visible = true -- แสดงเมนูหลักทันที
        isMenuOpen = true
    else
        -- ใส่คีย์ผิด! บินออกจากเกมทันที
